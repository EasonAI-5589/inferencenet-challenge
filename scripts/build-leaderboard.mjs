...
const widget = `<div class="paired-board" data-paired-leaderboard>
  <div class="paired-controls" hidden>
    <label><span>Configuration</span><select data-ranking-view aria-label="Filter configuration">${Object.entries(VIEWS).map(([key, label]) => `<option value="${key}">${escapeHTML(label)}</option>`).join('')}</select></label>
  ...
  <div class="table-scroll" tabindex="0" role="region" aria-label="Leaderboard results"><table class="paired-table">
    ...
    <thead><tr><th scope="col">#</th><th scope="col">Model</th><th scope="col">Harness</th>${Object.entries(METRICS).map(([key, label]) => `<th scope="col" data-metric="${key}" aria-sort="${key === DEFAULT_METRIC ? 'descending' : 'none'}">${label}</th>`).join('')}</tr></thead>
